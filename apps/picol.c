...
#define snprintf(_x, _xx, msg, n) printf(2, "" msg "\n", n)
...
l;    i->level = 0;
l; int x = malloc(sizeof(struct picolCallFrame));
l;    i->callframe = x;//malloc(sizeof(struct picolCallFrame));
l;    i->callframe->vars = NULL;
l;    i->callframe->parent = NULL;
l;    i->commands = NULL;
l;    i->result = strdup("");
}
...
l;    int j; char *name[] = {"+","-","*","/",">",">=","<","<=","==","!=", 0};
    for (j = 0; name[j]; j++)
l;        picolRegisterCommand(i,name[j],picolCommandMath,NULL);
l;    picolRegisterCommand(i,"set",picolCommandSet,NULL);
l;    picolRegisterCommand(i,"puts",picolCommandPuts,NULL);
l;    picolRegisterCommand(i,"if",picolCommandIf,NULL);
l;    picolRegisterCommand(i,"while",picolCommandWhile,NULL);
l;    picolRegisterCommand(i,"break",picolCommandRetCodes,NULL);
l;    picolRegisterCommand(i,"continue",picolCommandRetCodes,NULL);
l;    picolRegisterCommand(i,"proc",picolCommandProc,NULL);
l;    picolRegisterCommand(i,"return",picolCommandReturn,NULL);
}

int main(int argc, char **argv) {
    struct picolInterp interp;
	printf(1, "PICOL\n");
l;    picolInitInterp(&interp);
l;    picolRegisterCoreCommands(&interp);
    if (argc == 1) {
        while(1) {
            char clibuf[1024];
            int retcode;
            printf(1, "picol> "); fflush(stdout);
        ...
        char buf[1024*16];
        FILE *fp = fopen(argv[1], 0);
        if (!fp) {
            perror("open"); exit(1);
        }
        buf[fread(buf,1024*16,fp)] = '\0';
        fclose(fp);
        if (picolEval(&interp,buf) != PICOL_OK) printf(1, "%s\n", interp.result);
    }
    return 0;
}
